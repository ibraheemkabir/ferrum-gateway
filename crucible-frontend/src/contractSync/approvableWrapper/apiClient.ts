...
import { logError, ChainEventBase, UserContractAllocation, TokenDetails, ChainLogos, Utils, BackendConstants, STAKING_CONTRACTS_V_0_1, CRUCIBLE_CONTRACTS_V_0_1 } from "types";
...
import { CrucibleService } from "../service/crucibleService";
import { PROVIDERS } from "../utils/contracts";
...
export class ApiClient {
	...
	: Promise<UserContractAllocation> {
        ...
        const allocation = await instance.getContractAllocation(userAddress, contractAddress, currency)
        console.log(instance, allocation, 'alloallaoaoa')
        return await instance.getContractAllocation(userAddress, contractAddress, currency)
        // return this.api({
	...
	: Promise<string> {
        ...
        const requests1 = await instance.approveAllocationGetTransaction(currency, amount || '1', userAddress, contractAddress)
        console.log(requests1, 'requests1requests1')
        ...
        const requestId = await this.sendTransactionAsync( 
            this.network! as any, requests,
